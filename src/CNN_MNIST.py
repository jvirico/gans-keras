...
ckp_every_X_iterations = 2000
##########################################
##########################################

os.makedirs("checkpoints/2", exist_ok=True)
os.makedirs("images/2", exist_ok=True)
os.makedirs("plots/2", exist_ok=True)
...
def train(iterations, batch_size, sample_interval):
    ...
    for iteration in range(iterations):
        ...
        if((iteration+1) % iterations == 0 or ((iteration+1) < iterations and (iteration+1) % ckp_every_X_iterations == 0)):
            gan.save(checkpoint_dir + '/model2_' + str(iteration+1) + '.hdf5')
...
def sample_images(generator, iteration):
    ...
    fig.canvas.draw()
    plt.pause(0.01)
    if(save_images_to_disk): plt.savefig('./images/2/it_'+str(iteration)+'.png')
...
plt.ylabel("Loss")
plt.legend()
if(save_images_to_disk): plt.savefig('./plots/2/Losses_'+stamp+'.png')
...
plt.ylabel("Accuracy (%)")
plt.legend()
if(save_images_to_disk): plt.savefig('./plots/2/Accuracy_'+stamp+'.png')
plt.show()
